...
const Product = mongoose.model(
  ...
  new mongoose.Schema({
    ...
    packType: { type: String, required: true, minlength: 5, maxlength: 50 },
    ...
    orders: { type: Array, default: null, required: false },
  })
);
...
function validateProduct(product) {
  const schema = {
    ...
    packType: Joi.string().min(5).max(50).required(),
    ...
    parentCode: Joi.string().min(5).max(50),
